...
export default function SingleTodo({todo , setTodos , todos , index  }:Props) :React.ReactNode {
  ...
  return (
    <Droppable draggableId={todo.id.toString()} index={index}>

        {
            (provided)=>(

                <form className='todos_single' onSubmit={(e)=>{handelEdit(todo.id , e)}} {...provided.draggableProps} {...provided.dragHandleProps}  ref={provided.innerRef }>
                {
                    edit? 
                    <>
